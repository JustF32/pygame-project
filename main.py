...
class MainBoard:
    ...
    def check_cell2(self, player_pos):
        num_x = player_pos[0]
        num_y = player_pos[1]
        if self.width > num_x >= 0 and 0 <= num_y < self.height and self.board[num_x][num_y][0] == 'FINISH':
            if self.check_enemies():
                return True
            else:
                print('ENEMIES ARE THERE')
        else:
            return False

    def check_cell3(self, player_pos):
        num_x = player_pos[0]
        num_y = player_pos[1]
        if self.board[num_x][num_y][0] == 'WALL':
            return False
        else:
            return True
    ...
    def bowAttack(self, x, y):
        ...
        if mainB.board[cX][cY][0] == 'ENEMY':
            if cooldownAttack >= 2500:
                cooldownAttack = 0
                mainB.board[cX][cY][1].gethit(2, mainB)
                cdType = 'Attack'
    ...
    def move(self, x, y, board):
        ...
        if cooldownMove == 0:
            if mainB.check_cell((self.curX + x, self.curY - y)):
                ...
                pygame.display.flip()
                cdType = 'Move'
            if mainB.check_cell2((self.curX + x, self.curY - y)):
                winscreen()
            ...
            if cooldownAttack == 0:
                try:
                    if board.board[self.curX][self.curY - 1][0] == 'ENEMY':
                        board.board[self.curX][self.curY - 1][1].gethit(3, board)
                        cdType = 'Attack'
                    elif board.board[self.curX][self.curY + 1][0] == 'ENEMY':
                        board.board[self.curX][self.curY + 1][1].gethit(3, board)
                        cdType = 'Attack'
                    elif board.board[self.curX - 1][self.curY][0] == 'ENEMY':
                        board.board[self.curX - 1][self.curY][1].gethit(3, board)
                        cdType = 'Attack'
                    elif board.board[self.curX + 1][self.curY][0] == 'ENEMY':
                        board.board[self.curX + 1][self.curY][1].gethit(3, board)
                        cdType = 'Attack'
...
class EnemyWarrior(Enemy):
    ...
    def __init__(self, hp, x, y, board, img='SkeletEnemy.png'):
        super().__init__(hp, x, y, board, img)
...
class EnemyBowman(Enemy):
    def __init__(self, hp, x, y, board, img='ArcherEnemy.png'):
        super().__init__(hp, x, y, board, img)
...
class Wall2:
    def __init__(self, x, y, img):
        ...
        img = 'data/' + img
        self.imgage = pygame.image.load(img)
        self.imgage = pygame.transform.scale(self.imgage, (128, 128))
        mainB.set_cellNew((self.curX, self.curY), ['xd', self])
...
class pentagram(Wall):
    def __init__(self, x, y):
        super().__init__(x, y, 'pentagram.png')

    def render(self, screen, x, y):
        screen.blit(self.imgage, (x - 125, y - 260))
...
class Lake(Wall2):
    def __init__(self, x, y):
        super().__init__(x, y, 'lake.png')

    def render(self, screen, x, y):
        screen.blit(self.imgage, (x - 125, y - 64))
...
class Finish:
    def __init__(self, x, y, img):
        ...
        img = 'data/' + img
        self.imgage = pygame.image.load(img)
        self.imgage = pygame.transform.scale(self.imgage, (100, 100))
        mainB.set_cellNew((self.curX, self.curY), ['FINISH', self])

    def render(self, screen, x, y):
        screen.blit(self.imgage, (x - 100, y - 64))
...
r1 = random.randint(16, 19)
y2 = random.randint(19, 25)
r2 = random.randint(16, 19)
y3 = random.randint(19, 25)
y4 = random.randint(19, 25)
y5 = random.randint(19, 25)
r3 = random.randint(16, 19)
r4 = random.randint(16, 19)
r5 = random.randint(16, 19)
y6 = random.randint(19, 25)
...
skele5 = EnemyBowman(1, 15, 13, mainB)
...
GIGACHAD = EnemyWarrior(15, 24, 22, mainB, "BOSS1.png")
gc_folower = EnemyBowman(3, 25, 24, mainB, "archer_folower.png")
gc_folower1 = EnemyBowman(3, 21, 24, mainB, "archer_folower.png")
...
finish = Finish(34, 26, 'bonfire.png')
...
while running: # ивееентики))
    ...
    enemy5.clockTime()
    GIGACHAD.clockTime()
    gc_folower.clockTime()
    gc_folower1.clockTime()
    if cdType != 'None':
